  ...
  (when (executable-find "claude")
    ...
    (let ((claude-code-dir "~/.emacs.d/claude-code.el"))
      ...
      (when (file-exists-p claude-code-dir)
        ...
        ;; Set up keybindings using 'c' as the leader key
        (dl/leader-keys
          "c"   '(:ignore t :which-key "claude")
          "cc"  '(claude-code-menu :which-key "claude menu")
          "cn"  '(claude-code-create :which-key "new claude")
          "cs"  '(claude-code-send-region :which-key "send region")
          "cf"  '(claude-code-send-file :which-key "send file")
          "cb"  '(claude-code-send-buffer :which-key "send buffer")
          "ce"  '(claude-code-fix-error :which-key "fix error")
          "cr"  '(claude-code-restart :which-key "restart")
          "ck"  '(claude-code-kill :which-key "kill"))
        
        ;; Global keymap for quick access
