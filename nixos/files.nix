  ...
  xdg_stateHome  = "${home}/.local/state"; in
{
  ...
  "${xdg_configHome}/polybar/bin/search-nixos-updates.sh" = {
    ...
    text = ''
      ...
      /etc/profiles/per-user/${user}/bin/google-chrome-stable --new-window "https://search.nixos.org/packages?channel=22.11&from=0&size=50&sort=relevance&type=packages"
    '';
  };
