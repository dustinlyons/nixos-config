...
```sh
mkdir -p nixos-config && (cd nixos-config && nix flake '--extra-experimental-features nix-command' init -t github:dustinlyons/nixos-config#starter)
```

This is a full version with secrets management.
```sh
mkdir -p nixos-config && (cd nixos-config && nix flake '--extra-experimental-features nix-command' init -t github:dustinlyons/nixos-config#starterWithSecrets)
```
