...
> I encrypt these keys to `age` keys via `age-plugin-yubikey`. You should either create your own keys and name them exactly as I have or fork this repo and change how the `nix-command` manages key import (using KMS, CKM, paperkey, Hashicorp Vault, etc.). It's pretty simple `bash`.
