...
```sh
mkdir nixos-config && cd nixos-config && nix flake init -t github:dustinlyons/nixos-config#starter
```

This is a full version with secrets management.
```sh
mkdir nixos-config && cd nixos-config && nix flake init -t github:dustinlyons/nixos-config#starterWithSecrets
```
