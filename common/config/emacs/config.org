...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package projectile
    ...
    :init
      ...
      (when (system-is-linux)
        (setq projectile-project-search-path '("/home/dustin/.local/share/src")))
      (when (system-is-mac)
        (setq projectile-project-search-path '("/Users/dustin/.local/share/src")))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun dl/find-device-matching (string type)
  "Get the devices from `dl/get-ffmpeg-device' and look for a device
   matching `STRING'. `TYPE' can be :video or :audio."

    (let* ((devices (dl/get-ffmpeg-device))
  ...
  (defcustom dl/default-audio-device nil
    "The default audio device to use for whisper.el and outher audio processes."
    :type 'string)

  (defun dl/select-default-audio-device (&optional device-name)
    (interactive)
    (let* ((audio-devices (cadr (dl/get-ffmpeg-device)))
           (indexes (mapcar #'car audio-devices))
           (names (mapcar #'cdr audio-devices))
           (name (or device-name (completing-read "Select audio device: " names nil t))))
      (setq dl/default-audio-device (dl/find-device-matching name :audio))
      (when (boundp 'whisper--ffmpeg-input-device)
        (setq whisper--ffmpeg-input-device (format ":%s" dl/default-audio-device)))))

  (add-to-list 'load-path "/Users/dustin/.local/share/src/org-ai")
  (require 'org-ai)
  (add-hook 'org-mode-hook #'org-ai-mode)
  (org-ai-install-yasnippets) ;; if you are using yasnippet and want `ai` snippets
  (setq org-ai-openai-api-token "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")

  (add-to-list 'load-path "/Users/dustin/.local/share/src/whisper.el")
  (require 'whisper)
  (setq whisper-model "base"
        whisper-language "en"
        whisper-install-directory "/Users/dustin/.local/share/whisper"
        whisper-translate nil)
  (when (system-is-mac)
    (dl/select-default-audio-device "Dustin's Airpods")
    (when dl/default-audio-device)
      (setq whisper--ffmpeg-input-device (format ":%s" dl/default-audio-device)))
