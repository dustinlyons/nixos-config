...
#+BEGIN_SRC emacs-lisp
  ...
  (dl/leader-keys
    "a"  '(:ignore t :which-key "org-ai")
    "ae"  '(org-ai-explain-code :which-key "explain code")
    "as"  '(org-ai-summarize :which-key "summarize block")
    "ar"  '(org-ai-on-region :which-key "run prompt on region")
    "ac"  '(org-ai-refactor-code :which-key "refactor code"))

  (global-set-key (kbd "C-c y") 'whisper-run)
#+END_SRC
...
(defun dl/load-buffer-with-emacs-config ()
  ...
  (interactive)
  (find-file "~/.local/share/src/nixos-config/common/config/emacs/config.org"))

(defun dl/load-buffer-with-todo ()
  "Open the emacs configuration"
  (interactive)
  (find-file "~/.local/share/org-roam/20220419121404-todo.org"))

(defun dl/load-buffer-with-nix-config ()
  "Open the emacs configuration"
  (interactive)
  (find-file "~/.local/share/src/nixos-config/common/home-manager.nix"))

(defun dl/load-buffer-with-good-data-lol ()
  "Open the emacs configuration"
  (interactive)
  (find-file "~/.local/share/src/good-data-lol/apps/orchestrator/src/index.ts"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package projectile
    ...
    :init
      ...
      (when (system-is-linux)
        (setq projectile-project-search-path '("/home/dustin/.local/share/src/good-data-lol")))
      (when (system-is-mac)
        (setq projectile-project-search-path '("/Users/dustin/.local/share/src/good-data-lol")))
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'auto-mode-alist '("\\.html$" . web-mode)) ;; auto-enable for .js/.jsx files
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun fetch-add-feature-prompt ()
    (interactive)
    (let ((current-buffer-contents (buffer-string))
            (prompt-contents (with-temp-buffer
                            (insert-file-contents "~/.local/share/src/good-data-lol/prompts/feature-prompt.txt")
                            (buffer-string)))
            (project-root "~/.local/share/src/good-data-lol/apps/orchestrator"))
        (let* ((file1 (read-file-name "Select first file: " project-root))
            (file2 (read-file-name "Select second file: " project-root))
            (file3 (read-file-name "Select third file: " project-root))
            (file1-contents (with-temp-buffer
                                (insert-file-contents file1)
                                (let ((lines (split-string (buffer-string) "\n")))
                                (concat (mapconcat 'identity (seq-take lines 150) "\n")
                                        (when (> (length lines) 150) "\n...")))))
            (file2-contents (with-temp-buffer
                                (insert-file-contents file2)
                                (let ((lines (split-string (buffer-string) "\n")))
                                (concat (mapconcat 'identity (seq-take lines 150) "\n")
                                        (when (> (length lines) 150) "\n...")))))
            (file3-contents (with-temp-buffer
                                (insert-file-contents file3)
                                (let ((lines (split-string (buffer-string) "\n")))
                                (concat (mapconcat 'identity (seq-take lines 150) "\n")
                                        (when (> (length lines) 150) "\n...")))))
            (tree-output (shell-command-to-string "tree -L 2 ~/.local/share/src/good-data-lol | grep -v 'node_modules\\|tmp'"))
            (all-files (concat prompt-contents "\n" current-buffer-contents "\n\nResources:\n\n"
                                "---\n" file1 "\n---\n" file1-contents "\n\n"
                                "---\n" file2 "\n---\n" file2-contents "\n\n"
                                "---\n" file3 "\n---\n" file3-contents "\n\n"
                                tree-output)))
  ...
  (defun apply-clipboard-patch ()
    (interactive)
    (let* ((temp-file (make-temp-file "/tmp/clipboard-patch-" nil ".patch"))
           (clipboard-contents (current-kill 0)))
      (with-temp-file temp-file
        (insert clipboard-contents))
    (shell-command (concat "cd " (shell-quote-argument (file-name-directory (buffer-file-name))) " && patch < " (shell-quote-argument temp-file)))))

  (dl/leader-keys
    "c"  '(:ignore c :which-key "chatgpt")
    "cf" '(fetch-add-feature-prompt :which-key "add feature")
    "cp" '(apply-clipboard-patch :which-key "apply clipboard patch"))
#+END_SRC
